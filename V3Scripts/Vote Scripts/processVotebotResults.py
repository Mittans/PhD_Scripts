...
completedFolders = 'D:\PhD Work Code Backup\PipsqueakBackup'
    ...
    for y in paths:
        ...
        for line in csvtotalsBoostedFile:
            if episode not in ndictBoosted.keys():
                ndictBoosted[episode] = (int(line[2]), int(line[3]), int(line[4]), int(line[5]))
            else:
                ndictBoosted[episode] = tuple(map(operator.add, ndictBoosted[episode], (int(line[2]), int(line[3]), int(line[4]), int(line[5]))))
